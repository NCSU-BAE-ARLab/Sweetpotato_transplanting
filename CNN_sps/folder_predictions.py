...
def draw_predictions(image_np, outputs, score_thresh=0.7):
...
def run_inference_on_image(image_path, weights_path, model, device, save_path):
    ...
    keep_indices = ops.nms(outputs["boxes"], outputs["scores"], iou_threshold=0.5)
    outputs = {
...
def run_inference_on_folder(root_folder, weights_path, output_folder):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = get_model(weights_path=weights_path, device=device)
    
    image_extensions = [".jpg", ".jpeg", ".png", ".bmp"]
    for category in ["non-overlap", "overlap"]:
        input_dir = os.path.join(root_folder, category)
        ...
        for img_file in tqdm(image_files, desc=f"Inferencing {category}"):
            img_path = os.path.join(input_dir, img_file)
            save_path = os.path.join(output_dir, os.path.splitext(img_file)[0] + ".png")
            run_inference_on_image(img_path, weights_path, model, device, save_path)


if __name__ == "__main__":
    root_dataset = "../dataset_generation_sps/validation_data_annotated/non-leaf"
    output_results = "../results_blackB_old_slips"
    model_path = "../results_blackB_old_slips/maskrcnn_slip_1.pth"

    run_inference_on_folder(root_dataset, model_path, output_results)